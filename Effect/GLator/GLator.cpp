...
SmartRender(PF_InData *in_data, PF_OutData *out_data, PF_SmartRenderExtra *extra)
{
  ...
  if (!err){
    try
    {
      ...
      AESDK_OpenGL::AESDK_OpenGL_EffectRenderDataPtr 
      ...
      bool hasGremedy = renderContext->mExtensions.find(
        gl::GLextension::GL_GREMEDY_frame_terminator) != 
        renderContext->mExtensions.end();
      ...
      gl::GLenum glFmt;
      float multiplier16bit;
      gl::GLuint inputFrameTexture = UploadTexture(suites, format, input_worldP, 
                                                   output_worldP, in_data, pixSize, 
                                                   glFmt, multiplier16bit);
      
      
      // Set up the frame-buffer object just like a window.
      AESDK_OpenGL_MakeReadyToRender(*renderContext.get(), 
      ...
      RenderGL(renderContext, widthL, heightL, inputFrameTexture, 
               sliderVal, multiplier16bit);
      ...
      SwizzleGL(renderContext, widthL, heightL, 
                renderContext->mOutputFrameTexture, multiplier16bit);
