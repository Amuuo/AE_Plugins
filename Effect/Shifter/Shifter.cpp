    ...
    *outP = siP->pixelMap[xL][yL].pixel; 
    ...
    switch (siP->params[SORT_BY_DROPDOWN].u.pd.value) {
      ...
      case HUE: siP->in_data->utils->colorCB.Hue(
          siP->in_data->effect_ref, inP, &(siP->pixelMap[xL][yL].value)); break;
...
static PF_Err SmartRender(PF_InData* in_data, PF_OutData* out_data, PF_SmartRenderExtra* extra) {
  ...
  SorterBase* infoP = static_cast<SorterBase*>(suites.HandleSuite1()->
  ...
  if(!infoP->no_opB) {        
    ...
    if(!err && output_worldP) {
      ...
      ERR(suites.Iterate8Suite1()->iterate(in_data, 0, output_worldP->height, 
                                            ...
                                            (void*)(infoP), SortImagePixels, output_worldP));
...
static PF_Err PreRender(PF_InData* in_data,PF_OutData* out_data, PF_PreRenderExtra* extra) {
  ...
  infoH = suites.HandleSuite1()->host_new_handle(sizeof(SorterBase));    
  ...
  if (infoH) {
    ...
    if (!err) {
      ...
      SorterBase* base = static_cast<SorterBase*>(suites.HandleSuite1()->host_lock_handle(infoH));

      for (int i = 0; i < SORT_NUM_PARAMS; ++i) {
        PF_CHECKOUT_PARAM(in_data, i, in_data->current_time, in_data->time_step,
          in_data->time_scale, &base->params[i]);
      }
