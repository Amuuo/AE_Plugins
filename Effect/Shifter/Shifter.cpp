...
{  
  ...
  {    
    ...
    for (int j{0}, sortLength{0}; j < in_data->height; ++j, ++sortLength) 
    {                                                 
      if(mostQueue.empty()) 
      {
        rowBeginIters.clear();
        storeRowIters(i, j, rowBeginIters, shiftInfo);
        //sortBegin = shiftInfo->pixelMap[i].begin() + j;
        mostQueue.push(getColumnPixelAverage(i, j, shiftInfo));
        sortSteps2 = getMinSortLength(mostQueue.top(), shiftInfo);
      }
      else
      {                               
        if((std::abs(mostQueue.top()-getColumnPixelAverage(i, j, shiftInfo)) > 
           sortSteps2 && sortLength > minSortLength) || 
             j == shiftInfo->in_data.height - 1) 
        {       
          storeRowIters(i, j, rowEndIters, shiftInfo);
          //sortEnd = shiftInfo->pixelMap[i].begin() + j;
    ...
    {      
      ...
      if (!err && output_worldP) 
      {
        ...
        ERR(suites.WorldTransformSuite1()->blend(
            ...
            input_worldP,
            output_worldP,
            infoP->sortRangeBoosterSliderValue,
            output_worldP));
