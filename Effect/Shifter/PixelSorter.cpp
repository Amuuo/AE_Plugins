...
{ 
  ...
  {        
    current_segment.beginItems.push_back({shiftInfoCopy->pixelMap[j].begin()+pixelCounter, j, pixelCounter});
  }
}
...
{  
  for (PF_Fixed j = lineCounter; j < (lineCounter + param.sortWidth) && j < pixelLines - 1; ++j) 
  { 
    current_segment.endItems.push_back({shiftInfoCopy->pixelMap[j].begin()+pixelCounter, j, pixelCounter});
  }  
}
...
getLineWidthColorAverage()
{
  ...
  for (auto i = lineCounter; i<columnWidthSpan && i<pixelLines-1; ++i)
  {
    r_avg += static_cast<PF_FpLong>(shiftInfoCopy->pixelMap[i][pixelCounter].pixel.red);
    g_avg += static_cast<PF_FpLong>(shiftInfoCopy->pixelMap[i][pixelCounter].pixel.green);
    b_avg += static_cast<PF_FpLong>(shiftInfoCopy->pixelMap[i][pixelCounter].pixel.blue);
  }
  ...
  if (r_avg<current_segment.rgb_sort.low_value.red || current_segment.rgb_sort.low_value.red==0)
    current_segment.rgb_sort.low_value.red = r_avg;
  
  if (g_avg<current_segment.rgb_sort.low_value.green || current_segment.rgb_sort.low_value.green==0)
    current_segment.rgb_sort.low_value.green = g_avg;
  
  if (b_avg<current_segment.rgb_sort.low_value.blue || current_segment.rgb_sort.low_value.blue==0)
...
{
  if (needToReverse)
    reverse(current_segment.beginItems[index].beginIter, current_segment.endItems[index].endIter);
}
    ...
    case SORT_BY_LUMINOSITY:
      ...
      for (auto h = 0; h < current_segment.beginItems.size(); ++h)
      {
        ...
        switch (param.sortByColorChoice)
        {
          case RED_SORT:
          {
            PF_FpLong rgbBeginVal=static_cast<PF_FpLong>(current_segment.beginItems[h].beginIter->pixel.red);
            PF_FpLong rgbEndVal=static_cast<PF_FpLong>(current_segment.endItems[h].endIter->pixel.red);
            PF_FpLong rgbStep=(rgbBeginVal-rgbEndVal)/currPixDistance;

            for (auto pix=current_segment.beginItems[h].beginIter; pix!=current_segment.endItems[h].endIter; ++pix)
            {
              rgbBeginVal-=rgbStep;
          ...
          case GREEN_SORT:
          {
            PF_FpLong rgbBeginVal=static_cast<PF_FpLong>(current_segment.beginItems[h].beginIter->pixel.green);
            PF_FpLong rgbEndVal=static_cast<PF_FpLong>(current_segment.endItems[h].endIter->pixel.green);
            PF_FpLong rgbStep=(rgbBeginVal-rgbEndVal)/currPixDistance;
          ...
          case BLUE_SORT:
          {
            PF_FpLong rgbBeginVal=static_cast<PF_FpLong>(current_segment.beginItems[h].beginIter->pixel.blue);
            PF_FpLong rgbEndVal=static_cast<PF_FpLong>(current_segment.endItems[h].endIter->pixel.blue);
            PF_FpLong rgbStep=(rgbBeginVal-rgbEndVal)/currPixDistance;
